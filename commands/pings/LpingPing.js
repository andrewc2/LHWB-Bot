...
import { ApplicationCommandOptionType, EmbedBuilder, Colors, ComponentType } from 'discord.js';
import { database } from '../../models/database.js';
import Permission from '../../utilities/Permission.js';
import { autocomplete, checkPinglistExists, pingPinglistButton, joinPinglistButton, NO_MEMBERS, PINGLIST_NOT_FOUND } from '../../commandUtilities/lpingUtilities.js';
...
export default class LpingPing extends Command {
  ...
  async exec(interaction) {
    ...
    const handlePing = async () => {
      ...
      const [result] = await database.promise().query('SELECT u.userID FROM user as u INNER JOIN userPinglist as up ON u.userID = up.userID INNER JOIN pinglist as p ON p.pingID = up.pingID WHERE p.guildID = ? AND p.name = ?', [interaction.guild.id, pinglist]);
    ...
    if (!(await Permission.isTrustedMember(interaction.member))) {
