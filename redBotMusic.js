...
var firstJoin = true;
var versionNum = "2018.06.14a #music";
...
function isMod(channelID, userID){
    ...
    if (userID === creds.iandrewc || userID === creds.neonz || userID === creds.fs) return true; //lets iandrewc use his own bot
...
function restart(channelID, message, userID, user) {
    ...
    bot.sendMessage({to: channelID, message: "<@"+userID+">, LHWB Music restarting!"},
        function() {
            process.exit(-1);
        });
}
